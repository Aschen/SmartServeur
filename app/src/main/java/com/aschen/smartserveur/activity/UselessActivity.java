...
public class UselessActivity extends ActionBarActivity
{
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        if (extras != null)
        {
            int tableId = extras.getInt("table_id");
            RestAdapter adapter = new RestAdapter.Builder()
                                    .setEndpoint(SessionService.URL_API)
                                    .build();

            SessionService sessionService = adapter.create(SessionService.class);

            sessionService.createSession(new Session(false, tableId), new Callback<Session>()
            {
                @Override
                public void success(Session session, Response response)
                {
                    Intent accueil = new Intent(UselessActivity.this, AccueilTableActivity.class);
